"""Small offline tests for the 33-cell paper-metrics runner."""

import importlib.util
import json
from pathlib import Path

SCRIPT = Path(__file__).parents[1] / "experiments" / "20_paper_metrics_panel.py"
spec = importlib.util.spec_from_file_location("paper_panel", SCRIPT)
paper_panel = importlib.util.module_from_spec(spec)
spec.loader.exec_module(paper_panel)


def test_discovery_uses_saved_identity_and_checkpoint_convention(tmp_path, monkeypatch):
    monkeypatch.setattr(paper_panel, "EXPECTED_CELLS", 1)
    monkeypatch.setattr(paper_panel, "EXPECTED_MODELS", 1)
    cell = tmp_path / "sweep" / paper_panel.TAG / "model_a" / "seed_42"
    cell.mkdir(parents=True)
    config = {
        "model_name": "example/base",
        "base_revision_requested": "commit123",
        "seed": 42,
        "contexts_sha256": paper_panel.context_hash(),
    ...
    found = paper_panel.discover_cells(tmp_path / "sweep", tmp_path / "checkpoints")
...
def test_resume_requires_complete_record_and_exact_identity(tmp_path):
    ...
    paper_panel.save_json(path, {"complete": True, "identity": identity})
    assert paper_panel.reusable_result(path, identity)["complete"] is True
    assert paper_panel.reusable_result(path, {"identity_sha256": "different"}) is None
...
def test_short_row_contains_no_alignment_score():
    ...
    row = paper_panel.short_row(record)
    assert row["B_T"] == 0.4
...
def test_json_hash_is_independent_of_dictionary_order():
    assert paper_panel.json_hash({"a": 1, "b": 2}) == paper_panel.json_hash(
        {"b": 2, "a": 1}
    )
